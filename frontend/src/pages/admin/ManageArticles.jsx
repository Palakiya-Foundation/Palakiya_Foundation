...
const ManageArticles = () => {
  ...
  const openEdit = (a) => {
    ...
    const existingAuthorIds = Array.isArray(a.authors)
      ? a.authors.map((x) => Number(x.authorId || x.id)).filter(Boolean)
      : [];
    ...
    setForm({
      ...
      content: a.content,
      category: a.category,
      // keep legacy field (backend still has it)
      author: a.author,
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => {
        if (k === 'authorIds') {
          // backend supports array or JSON string
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      // no photo update in this popup (can be extended)

                      await api.put(`/authors/${selectedAuthor.id}`, data);
                      // refresh author details + keep editor values in sync
